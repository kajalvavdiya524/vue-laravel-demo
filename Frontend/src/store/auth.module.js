...
const actions = {
  ...
  async login({ commit, rootGetters }, { email = null, password = null, syncUser = null }) {
    if (!syncUser) {
      commit('loginRequest');
    ...
    try {
      const user = syncUser != null ? syncUser : await UserService.login(email, password);
  ...
  async logout({ commit }, { forced = false, stay = false, moveTo = { name: 'login' } }) {
