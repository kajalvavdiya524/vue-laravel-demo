<template>
  <div class="component-hotels">
        ...
        <ValidationObserver ref="form" slim>
          ...
          <div class="row">
            ...
            <div class="col-12 col-md-6">
              ...
              <ValidatedField name="zip" type="number" purenumber integer
                              :disabled="pending" rules="required|numeric" v-model.trim="hotel.zip"/>
            </div>
          </div>
          <div class="row">
            <div class="col-12 col-md-6">
          ...
          <div class="row">
            ...
            <div class="col-12 col-md-6">
              ...
              <drop-down id="currency" :items="allCurrencies" :disabled="pending" v-model="hotel.currency_id" />
            </div>
          </div>
    ...
    <div class="position-relative panel-content" :class="{ panel: !admin }">
      <div class="list d-none d-md-block" :class="{ 'left-edge': !admin }">
          ...
          <table class="w-100">
            <thead>
              <tr>
                ...
                <th class="w-name">
                  {{ $t('pages.hotels.headers.name') }}<sort-indicator v-model="sort" field="name" />
                </th>
                <th class="w-name">
                  {{ $t('addr.country') }}<sort-indicator v-model="sort" field="country.name" />
                </th>
                <th class="w-name">
                  {{ $t('pages.hotels.headers.currency') }}<sort-indicator v-model="sort" field="currency.code" />
                </th>
                <th class="w-name">
                  {{ $t('pages.hotels.headers.create-date') }}<sort-indicator v-model="sort" field="created_at" />
                </th>
                <th class="w-name">
                  {{ $t('pages.hotels.headers.status') }}<sort-indicator v-model="sort" field="active" />
                </th>
        ...
        <div v-if="loaded" class="d-flex justify-content-between">
          ...
          <b-btn pill variant="secondary" class="add-new-hotel" v-if="admin"
      ...
      <p class="head-line d-md-none"><span class="d-none">{{ heading }}</span><spinner v-if="!loaded" /></p>
      <div class="d-md-none list-item" v-if="!hasHotels">
        <div class="w-empty">{{ $t('pages.hotels.no-hotels') }}</div>
      </div>
      <div class="d-md-none list-item" v-for="row in hotels" :key="row.id">
        <div class="d-flex">
          <div class="c-1 d-flex">
            <div class="w-25">
        ...
        <div class="d-flex line">
          <div class="w-33">
            ...
            <p class="status" :class="{ ok: row.active }">
              {{ $t(`pages.hotels.status-${row.active?'':'de'}activated`) }}
            </p>
          </div>
    ...
    <div class="d-md-none add-new-hotel-block d-flex" v-if="loaded">
      ...
      <b-btn v-if="admin" pill variant="secondary" class="add-new-hotel" @click="openImportForm">
        <icon width="10" height="10" type="plus"/>
        {{ $t('pages.hotels.button-import') }}
      </b-btn>
    </div>
