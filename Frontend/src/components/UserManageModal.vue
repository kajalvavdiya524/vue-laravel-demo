    ...
    computed: {
      modalTitle() {
        if (this.user.id == null) {
          return this.$t('pages.users.modal.newUser.title-add');
        }
      ...
      userFormInvalid() {
        ...
        if (this.user.id == null) return invalid;
        return Object.keys(errors).some((k) => errors[k].length > 0);
      },
    },
    methods: {
      modalScroll(ev) {
      ...
      submitForm() {
        ...
        if (user.id != null && !user.password.length) {
