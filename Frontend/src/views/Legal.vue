<template>
  <div class="page-legal">
    <template v-if="main">
      ...
      <div class="pb-2">
        <p v-for="{ id, slug } in legalPages" :key="`link-${slug}`">
          <router-link
            :to="{ name: 'legal', params: { page: slug } }"
          >{{ $t(`pages.legal.pages.${id}`) }}</router-link>
        </p>
      </div>
    </template>
    ...
    methods: {
      updatePage() {
        ...
        this.$nextTick(() => {
          ...
          if (page == null) return;
          const title = this.$t(`pages.legal.pages.${page.id}`);
          this.page = {
            id: page.id,
            title,
          };
