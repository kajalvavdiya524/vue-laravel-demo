<template>
  <div class="page-reservations" id="page-reservations">
    ...
    >
      ...
      <div class="columns">
        ...
        <div class="notes">
          ...
          <p v-if="!remarks(shownRecord, 'booker')"><i>{{ $t('pages.reservations.person-modal.no-remarks') }}</i></p>
          ...
          <p v-if="!remarks(shownRecord, 'room')"><i>{{ $t('pages.reservations.person-modal.no-remarks') }}</i></p>
          ...
          <p v-if="!remarks(shownRecord, 'channel')"><i>{{ $t('pages.reservations.person-modal.no-remarks') }}</i></p>
    ...
    <b-modal
      ...
      hide-footer
      v-if="creditCardDetails.id != null"
      @hidden="creditCardDetails = {}"
    >
      <template #modal-header-close>
        <icon width="20" height="20" class="d-none d-md-block" type="times"/>
        <icon width="10" height="18" class="d-md-none" type="arrow-left"/>
      ...
      <p class="text-md">
        <spinner v-if="!creditCardIframe.loaded" />
        <iframe
          :src="creditCardIframe.src"
          v-show="creditCardIframe.loaded"
        ></iframe>
      </p>
    </b-modal>
      ...
      <div class="list-item" v-for="row in sortedList" :key="row.id">
        ...
        <div class="d-flex">
          ...
          <div class="w-50 pad-left">
            ...
            <p class="credit-card">{{ $t('pages.reservations.button-details') }}</p>
          </div>
        </div>
        ...
        <table class="w-100">
          <thead>
            <tr>
              ...
              <th class="text-right">
                {{ $t('pages.reservations.headers.channel') }}<br>{{ $t('pages.reservations.headers.bookingid') }}
              </th>
            </tr>
          </thead>
...
<script>
  ...
  import { creditCardDetailsIframe } from '@/shared';
  ...
  export default {
    ...
    computed: {
      ...
      searchForItems() {
        return [
          ...
          { id: 1, text: this.$t('pages.reservations.search-for-options.check-in'), disabled: true },
          { id: 2, text: this.$t('pages.reservations.search-for-options.check-out'), disabled: true },
        ];
      },
      sortedList() {
    ...
    methods: {
      ...
      prepareCreditCardModal(row) {
        this.creditCardDetails = row;
        this.creditCardIframe.loaded = true;
        this.creditCardIframe.src = `${creditCardDetailsIframe}bookingID=${this.creditCardDetails.source.id}&objectID=${this.creditCardDetails.id}&userID=${this.userID}&css=null`;
      },
    },
  };
</script>
