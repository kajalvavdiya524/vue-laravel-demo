<template>
  <div class="page-users">
    ...
    <b-modal
      ...
      :no-close-on-backdrop="usersUpdatePending"
    ...
    <b-modal
      ...
      :no-close-on-backdrop="rolesUpdatePending"
                ...
                <table class="w-100" v-if="usersLoaded">
                  ...
                  <tbody v-for="row in filteredUsers" :key="row.email">
                    ...
                    <tr>
                      ...
                      <td :class="{ 'font-italic': row.group_owner }">
                        {{ roleNames(row) }}
                      </td>
                      <td class="actions">
                        <b-btn
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('user', ['userID', 'hotelID', 'hotels', 'allowedPages', 'allowedUserPages']),
      ...
      allowedRoles() {
        return this.roles.map(({ id, name: text }) => ({ id, text }));
      },
      languages() {
        return this.langs.map((code) => ({ id: code, text: this.$t(`langs.${code}`) }));
      },
      roleFormInvalid() {
        return this.$refs.roleForm && this.$refs.roleForm.flags.invalid;
      },
      inviteFormInvalid() {
        return this.$refs.inviteForm && this.$refs.inviteForm.flags.invalid;
      },
      roleIsAllPages() {
    ...
    methods: {
      ...
      protectedUser(user) {
        return user.group_owner || user.id === this.userID;
      },
      userName(user) {
        return user.profile != null ? user.profile.name : this.$t('pages.users.invite-sent');
      },
      roleNames(row) {
        if (row.group_owner) return this.$t('pages.users.group-owner');
