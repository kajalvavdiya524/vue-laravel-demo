...
//  return (new VerifyEmailNotification())->toMail($user);
  return (new ResetPasswordNotification('123'))->toMail($user);
});
...
//  app()->setLocale('ru');
//  return (new VerifyEmailNotification())->toMail($user);
  return (new \App\Notifications\InviteUserNotification('123'))->toMail($user);
});
...
Route::get('/hotels/{id?}', function (PMSManager $manager, $id) {
  /** @var Hotel $hotel */
...
Route::get('/description', function(PMSManager $manager) {
  $user = User::find(1);
  $payload = [];
  $res = $manager->debug()->getDescription($user->profile, $payload);
  return debug_response($res);
});
