...
class ImagesController extends Controller {
  ...
  public function create(Request $request, PMSManager $manager)
  {
    ...
    <FileMetaDataRQ>
        <pos>
            <source agentsine="74" agentdutycode="18941305ba1e3572" userid="'.$user->id.'" />
            <BookingChannel Type="4" />
        </pos>
        <AccessType type="0" level="0"/>
        <RetainFileName status="0" />
        <OverWrite status="1"/>
        <FixedPathReservation path="'.$file['tmp_name'].'" key="abc"/>
        <MetaData>
            <FileName>'.$file['name'].'</FileName>
            <Details size="'.$file['size'].'" type="'.$file['type'].'"/>
            <Category name="Test category" id="123"/>
            <Description>This is sample file</Description>
        </MetaData>
    </FileMetaDataRQ>';
    $xmlArray = [];
    $xmlArray['textData'] = urlencode($xml);
    $xmlArray['xmlFile'] = new \CurlFile($_FILES['image']['tmp_name']);
    $this->sendXmlRequestToCML($xmlArray);
    // ImageResource::withoutWrapping();
    // return ImageResource::make($image);
  }

  public function sendXmlRequestToCML($xmlArray = array()) {
    $xml  = new \DOMDocument();

    $cmlURL = 'http://cms.cultuzz.com:8080/CMS/MediaUploaderServlet?ip=172.25.0.1';
    var_dump($cmlURL);
    $curlHandle = curl_init($cmlURL);
    if (curl_errno($curlHandle)) {
        ...
        curl_setopt($curlHandle, CURLOPT_POSTFIELDS, $xmlArray);
        curl_setopt($curlHandle, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curlHandle, CURLOPT_SSL_VERIFYHOST, false);
        curl_setopt($curlHandle, CURLOPT_SSL_VERIFYPEER, false);
        $curlResponse = curl_exec($curlHandle);
        echo json_encode($curlResponse, true);
