...
class PMSManager {
  ...
  private function sendRequest(XMLElement $xml, $params = [])
  {
    ...
    if($authBasic || $authSign) {
      ...
      $endpoint = $this->config['endpoint_collab'].$isCollab;
      $http = Http::asForm();
      $form_params = [$isCollab => $content];
      $httpParams = compact('form_params');
    } elseif ($isMappings) {
  ...
  public function getRooms($withImages = false)
  {
    $xml = XmlElement::createRoot('OTA_HotelDescriptiveInfoRQ', function (XmlElement $el) {
      ...
      $el->add('HotelDescriptiveInfos')
        ...
        ->add('ContentInfos')
        ->add('ContentInfo', ['Name' => 'ProductElement']);
    });

    try {
      $res = $this->sendRequest($xml);
      return $this->extractRoomTypes($res, $withImages);
    } catch(Throwable $e) {
  ...
  private function parseRoomType(XmlElement $plan, $addImages = false, $short = false)
  {
    ...
    if (!$short) {
      ...
      $item += $this->extractValidities($plan);
      if ($addImages) {
        $item += [
      ...
      $el->add('RatePlans')
        ->add('RatePlan', $ratePlanAttributes, function (XmlElement $el) use ($payload, $isCreate, $isCopy, $isDelete, $isUpdate) {
          ...
          $el->add('Rates', function (XmlElement $el) use ($payload) {
            ...
            // validity
            $from = $this->carbonDate(Arr::get($payload, 'validity.from'));
            if(Arr::get($payload, 'validity.unlim')) {
              $until = $from->clone()->addYears(self::VALIDITY_UNLIMITED_YEARS);
            } else {
              $until = $this->carbonDate(Arr::get($payload, 'validity.until'));
            }
            $el->add('Rate', [
...
//      throw new Error('Invalid params in .getRoomsData');
    ...
    try {
      ...
      // update hotel's currency, if needed
      $this->hotel->checkCurrency($currency);
      $currency = $this->hotel->currency ?? Currency::findByCode('EUR');
        ...
        ->add('RateAmountMessage', function (XmlElement $el) use ($payload) {
          ...
          $el->add('Rates', function (XmlElement $el) use ($payload, $start) {
            ...
            $el->add('Rate')
              ...
              ->add('BaseByGuestAmt', [
                'DecimalPlaces'  => '2',
                'CurrencyCode'   => $this->hotel->currency->code ?? 'EUR',
  ...
  private function parseReservation(SimpleXMLElement $reservation)
  {
    ...
    foreach($pids as $pid) {
      ...
      $rooms[$rid] += compact('from', 'till', 'pid', 'rid', 'price', 'cur');
    }
  ...
  private function extractValidities(XmlElement $plan)
  {
    $ret = [
      'validity'  => [],
      'blockouts' => [],
...
//           $el->add('ContentInfo', ['Name' => 'AllPoliciesList']);
          ...
          $el->add('ContentInfo', ['Name' => 'Product']);
//           $el->add('ContentInfo', ['Name' => 'Product', 'Code' => '145897']); // 145895
//           $el->add('ContentInfo', ['Name' => 'ProductElementList', 'Code' => '0']);
          $el->add('ContentInfo', ['Name' => 'ProductElement']);
//           $el->add('ContentInfo', ['Name' => 'ProductElement', 'Code' => '81781']);
//           $el->add('ContentInfo', ['Name' => 'ProductList']);
...
//      return $res->asXML();
      ...
      $this->hotel->checkCurrency(Arr::get($_, 'currency.code'));
  ...
  public function createRatePlanFromRoomType($data)
  {
    ...
    $payload = [
      ...
      'validity'  => Arr::get($data, 'validity'),
        ...
        ->add('RatePlan', $ratePlanAttributes, function (XmlElement $el) use ($payload, $isCopy, $isDelete, $priceVariationId) {
          ...
          foreach([
            ...
            $el->add('Description', ['Name' => $x], function (XmlElement $el) use ($payload, $k) {
              foreach(self::$langs as $lang) {
                $el->add('Text', ['Language' => $lang], (Arr::get($payload, "langs.$lang.$k") ?? ''));
              }
            });
          }
          // prices
...
//    }
  ...
  {
    $country = Country::normalize(Arr::get($payload, 'country', Arr::get($payload, 'country_id')));
    $currency = Currency::normalize(Arr::get($payload, 'currency', Arr::get($payload, 'currency_id')));
    $payload['country'] = $country;
    $payload['currency'] = $currency;
    Arr::forget($payload, ['country_id', 'currency_id']);
      ...
      $el->add('Profile', [
        ...
        $el->add('CompanyInfo', function (XmlElement $el) use ($payload) {
          ...
          $el->add('AddressInfo', ['Type' => '2', 'UseType' => '12'], function (XmlElement $el) use ($payload) {
            ...
            $el->StateProv = 'NA';
            $el->add('CountryName', ['Code' => Arr::get($payload, 'country.code')], Arr::get($payload, 'country.name'));
          });
    ...
    try {
      ...
      $data = compact('id', 'name', 'ctx', 'user_id', 'currency_id', 'country_id', 'group_id');
      $hotel = Hotel::create($data);
      // update currency

      try {
        $this->modifyHotel(Arr::only($payload, 'currency'), $hotel);
      } catch (Throwable $e) {
        // do nothing
      }
      return $hotel;
  ...
  public function getHotel(Hotel $hotel = null, $debug = false)
  {
    ...
    try {
      ...
      return $this->parseHotelData($info);
    } catch(Throwable $e) {
  ...
  public function modifyHotel($payload, $hotel = null)
  {
    ...
    $xml = XmlElement::createRoot('OTA_HotelDescriptiveContentNotifRQ', function (XmlElement $el) use ($payload, $hotel) {
      ...
      $el->add('HotelDescriptiveContents', function (XmlElement $el) use ($payload, $hotel) {
        ...
        if ($currency = Arr::get($payload, 'currency.code')) {
          ...
          $el->add('HotelInfo', $this->hotelCodeContext($hotel), function (XmlElement $el) use ($payload, $hotel) {
            $el->add('Descriptions', function (XmlElement $el) use ($payload, $hotel) {
              $el->add('Description', ['ContentCode' => '8', 'Name' => 'txt:HotelName'])
                ->add('Text', ['Language' => 'en'], Arr::get($payload, 'name', $hotel->name));
              $el->add('Description', ['ContentCode' => '4', 'Name' => 'img:company_logo'])
                ->add('Text', ['Language' => 'en'], $hotel->logo);
              ...
              if ($descriptions = Arr::get($payload, 'descriptions')) {
                foreach (Cultuzz::DESCRIPTION_CODE_NAMES as $code => $name) {
                  $el->add('Description', [
                    ...
                    foreach (Arr::get($descriptions, $name.'.lang', []) as $lang => $value) {
            ...
            if ($position = Arr::get($payload, 'position') && isset($position['latitude'], $position['longitude'])) {
              $el->add('Position', ['Longitude' => $position['longitude'], 'Latitude' => $position['latitude']]);
            }
          });
...
//                }
  ...
  private function parseHotelData(XmlElement $el)
  {
    $_ = (string)$el['CurrencyCode'];
    $cur = Currency::findByCode($_);
    if (!$cur) $cur = Currency::findByCode('EUR');
    $currency_id = $cur->id;
    ...
    $_ = (string)$addr->CountryName['Code'];
    $country = optional(Country::where('code', $_)->first())->code;
    ...
    return compact(
      'currency_id', 'name', 'active', 'capacity', 'capacity_mode', 'latitude', 'longitude', 'website',
      'street', 'street_optional', 'city', 'zip', 'country', 'tel', 'email', 'altemail'
    );
  }
              ...
              $el->add('RequiredPaymts')
                ->add('GuaranteePayment', $guaranteePaymentAttributes, function (XmlElement $el) use ($payload) {
                  ...
                  foreach([
                    ...
                    $el->add('Description', ['Name' => $x], function (XmlElement $el) use ($payload, $k) {
                      foreach(self::$langs as $lang) {
                        $el->add('Text', ['Language' => $lang], (Arr::get($payload, $k) ?? ''));
                      }
                    });
                  }
                });
...
//    }
  ...
  public function channelRatesList($id, $extractTypes = true)
  {
...
//        $channels['fields'] = $this->extractChannelFields($res, $id);
        ...
        $ids = $channels->where('type', 'push')->pluck('id')->toArray();
        $allFields = $this->channelsFields($ids);
...
//      $ret['url'] = (string)$info['URL'];
      ...
      if ($RegInfo) {
        ...
        $period = [
          ...
          ] + $this->extractDuration($RegInfo->first('./Periods/Period[@Code="Contract"]/Deadline'));
        $period['type'] = isset($period['until']) ? 1 : 0;
        $ret['period'] = $period;
      }
      $this->extractChannelValues($info, $fields, $ret);
      if (!Arr::exists($ret, 'period')) {
        $ret['period'] = ['until' => null, 'type' => 0, 'unit' => 'd', 'number' => 0];
      }
...
//    }
  ...
  private function extractChannelFields(XmlElement $el, $id = null)
  {
    $xp = './/HotelInfo/Services/Service';
    if ($id) $xp.= '[@Code="'.$id.'"]';
    $ret = collect($el->xpath($xp))->mapWithKeys(function (XmlElement $service) {
      $id = $service->stringAttr('Code');
      $fields = $this->parseChannelFields($service);
      return [$id => $fields];
    });
  ...
  private function parseChannelFields(XmlElement $service)
  {
    return collect($service->xpath('./Features/Feature'))->map(function (XmlElement $feature) {
      $key = $feature->stringAttr('CodeDetail');
      $code = $feature->stringAttr('ExistsCode');
      if (!Cultuzz::isValidFeature($code)) return null;
  ...
  private function extractChannelValues(XmlElement $info, $fields, &$appendTo = null)
  {
    ...
    $number = $info->first('.//Feature[@ExistsCode="27"]/Description/@ContentCaption');
    if (isset($number)) {
      $unit = $info->first('.//Feature[@AccessibleCode="27"]/Description/@ContentCaption');
      if (isset($unit)) {
        $ret['period'] = [
      ...
      if (isset($duration)) {
        $ret['period'] = [
          ...
          'unit'   => 'd',
          'type'   => 1,
          'until'  => $this->carbonDate(explode('T', (string)$duration)[0])->format('Y-m-d')
        ];
      }
    }
    if ($fields) {
    ...
    $xml = XmlElement::createRoot('OTA_HotelDescriptiveContentNotifRQ', function (XmlElement $el) use ($payload, $fields) {
      ...
      $el->add('HotelDescriptiveContents', $this->hotelCodeContext())
        ...
        ->add('ChannelInfo', $attrs, function (XmlElement $el) use ($payload, $fields) {
          if ($fields) {
            $UserName = $this->config['push_channels_login'];
            $Oldpassword = $Newpassword = $this->config['push_channels_password'];
            $RegInfo = $el->add('RegistrationInfo', compact('UserName', 'Oldpassword', 'Newpassword'));
            $periodType = Arr::get($payload, 'period.type');
            if ($periodType == 0) {
              $RegInfo->add('Periods', function (XmlElement $el) use ($payload) {
                ...
                $el->add('Period', ['CodeContext' => 'Contract'])->add('Deadline', $attrs);
              });
            } elseif ($periodType == 1) {
...
//      return $res->asXML();
  ...
  public function modifyPushMappings($payload)
  {
    ...
    $crates = $this->channelRatesList(Arr::get($payload, 'id'), false);
    ...
    $xml = XmlElement::createRoot('OTA_HotelRatePlanNotifRQ', function (XmlElement $el) use ($payload, $rooms, $crates) {
      ...
      $el->add('RatePlans', function (XmlElement $el) use ($payload, $rooms, $crates) {
        ...
        foreach($rooms as $room) {
          ...
          if (!$crate = Arr::get($crates, $uniq)) return;
          $rp = $el->add('RatePlan', [
            'RatePlanID'                 => $room['rid'],
            'RatePlanNotifType'          => 'Overlay',
            'RatePlanType'               => '11',
...
//      return $_->asXML();
  ...
  private function parseChannelMappings(XmlElement $el = null, $extractTypes = true)
  {
    ...
    $rates = array_map(function (XmlElement $el) {
      $type = (string)$el['InvType'];
      $typeid = (string)$el['InvCode'];
      $id = (string)$el['InvGroupingCode'];
      $name = (string)$el->Description['Name'] ?? '';
      $uniq = "{$id}_{$typeid}";
      return compact('id', 'name', 'type', 'typeid', 'uniq');
    }, $el->xpath('//RatePlans/RatePlan[not(@MarketCode)]/SellableProducts/SellableProduct[@InvCode][@InvGroupingCode][@InvType]'));
...
//      return $res->asXML();
  ...
  public function getDescription($payload = [])
  {
    $xml = XmlElement::createRoot('OTA_HotelDescriptiveInfoRQ', function (XmlElement $el) use ($payload) {
      $this->addPOS($el);
      $el->add('HotelDescriptiveInfos', $this->hotelCodeContext())
        ->add('HotelDescriptiveInfo', $this->hotelCodeContext())
        ->add('HotelInfo', ['SendData' => 1]);
    });
    ...
    try {
      ...
      return $this->parseDescriptions($res);
    } catch (Throwable $e) {
      $this->throwError(__FUNCTION__, $e, "Failed to fetch what's nearby.", $payload);
    }
  }
  ...
  private function parseDescriptions(XmlElement $data): ?array
  {
    ...
    foreach (Cultuzz::DESCRIPTION_CODE_NAMES as $code => $name) {
      $languages = collect($data->xpath(sprintf('//Descriptions/Description[@ContentCode="%s"]/Text', $code)))
        ->mapWithKeys(fn($lang) => [(string)$lang['Language'] => (string)$lang]);

      $result[strtolower($name)]['lang'] = count($languages) ? $languages : null;
    }
  ...
  public function setActivePMS($payload = null): bool
  {
    ...
    $xml = XmlElement::createRoot('OTA_HotelDescriptiveContentNotifRQ', function (XmlElement $el) use ($payload) {
      ...
      $hotelAttrs = [
          ...
          'CurrencyCode' => Arr::get($payload, 'currency', $this->hotel->currency->code ?? 'EUR'),
